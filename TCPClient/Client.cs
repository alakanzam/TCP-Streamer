using System;
using System.Net.NetworkInformation;
using System.Net.Sockets;
using System.Threading;

namespace NF
{
    public class TCPClient
    {
        public delegate void DelegateConnection(TCPClient client, string Info);


        //Delegates bzw. Events
        public delegate void DelegateDataReceived(TCPClient client, byte[] bytes);

        public delegate void DelegateDataReceivedComplete(TCPClient client, string message);

        public delegate void DelegateDataSend(TCPClient client, byte[] bytes);

        public delegate void DelegateException(TCPClient client, Exception ex);

        private readonly Locker_AutoConnectClass Locker_AutoConnect = new Locker_AutoConnectClass();
        private readonly int m_Port;
        private readonly string m_Server;

        //Attribute
        public TcpClient Client;

        private bool m_AutoConnect;
        private int m_AutoConnectInterval = 10;
        private byte[] m_ByteBuffer;
        private NetworkStream m_NetStream;
        private Timer m_TimerAutoConnect;

        /// <summary>
        ///     Konstruktor
        /// </summary>
        public TCPClient(string server, int port)
        {
            //Daten setzen
            m_Server = server;
            m_Port = port;

            //Events anhängen
            ExceptionAppeared += OnExceptionAppeared;
            ClientConnected += OnConnected;
            ClientDisconnected += OnDisconnected;
        }

        /// <summary>
        ///     Interval für AutoConnect in Sekunden
        /// </summary>
        public int AutoConnectInterval
        {
            get => m_AutoConnectInterval;
            set
            {
                m_AutoConnectInterval = value;

                //Prüfen
                if (value > 0)
                    try
                    {
                        //Wenn schnon aktiv
                        if (m_TimerAutoConnect != null)
                            m_TimerAutoConnect.Change(value * 1000, value * 1000);
                    }
                    catch (Exception ex)
                    {
                        ExceptionAppeared(this, ex);
                    }
            }
        }

        /// <summary>
        ///     Regelt die Automatische Wiederverbindung
        /// </summary>
        /// <returns></returns>
        public bool AutoConnect
        {
            get => m_AutoConnect;
            set
            {
                m_AutoConnect = value;

                if (value)
                    InitTimerAutoConnect();
            }
        }

        /// <summary>
        ///     Gibt an ob der Client versucht sich über AutoConnect zu verbinden
        /// </summary>
        public bool IsRunning => m_TimerAutoConnect != null;

        /// <summary>
        ///     Gibt an ob der Client verbunden ist. Readonly
        /// </summary>
        /// <returns></returns>
        public bool Connected
        {
            get
            {
                if (Client != null)
                    return Client.Connected;
                return false;
            }
        }

        /// <summary>
        ///     ToString
        /// </summary>
        /// <returns></returns>
        public override string ToString()
        {
            return string.Format("{0} {1}:{2}", GetType(), m_Server, m_Port);
        }

        public event DelegateDataReceived DataReceived;
        public event DelegateDataSend DataSend;
        public event DelegateConnection ClientConnected;
        public event DelegateConnection ClientDisconnected;
        public event DelegateException ExceptionAppeared;

        /// <summary>
        ///     Timer für AutoConnect initialisieren
        /// </summary>
        private void InitTimerAutoConnect()
        {
            //Wenn AutoConnect
            if (m_AutoConnect)
                if (m_TimerAutoConnect == null)
                    if (m_AutoConnectInterval > 0)
                        m_TimerAutoConnect = new Timer(OnTimer_AutoConnect, null, m_AutoConnectInterval * 1000,
                            m_AutoConnectInterval * 1000);
        }

        /// <summary>
        ///     Daten senden
        /// </summary>
        /// <param name="Data"></param>
        public void Send(byte[] data)
        {
            try
            {
                // Sende den kodierten string an den m_Server
                m_NetStream.Write(data, 0, data.Length);

                //Event abschicken
                if (DataSend != null)
                    DataSend(this, data);
            }
            catch (Exception ex)
            {
                //Exception Event abschicken
                ExceptionAppeared(this, ex);
            }
        }

        /// <summary>
        ///     Startet das Lesen
        /// </summary>
        /// <param name="Data"></param>
        private void StartReading()
        {
            try
            {
                m_ByteBuffer = new byte[1024];
                m_NetStream.BeginRead(m_ByteBuffer, 0, m_ByteBuffer.Length, OnDataReceived, m_NetStream);
            }
            catch (Exception ex)
            {
                //Exception Event abschicken
                ExceptionAppeared(this, ex);
            }
        }

        /// <summary>
        ///     Wird aufgerufen wenn Daten erhalten wurden
        /// </summary>
        /// <param name="result"></param>
        private void OnDataReceived(IAsyncResult ar)
        {
            try
            {
                //Netzwerkstream ermitteln
                var myNetworkStream = (NetworkStream) ar.AsyncState;

                //Networkstream prüfen
                if (myNetworkStream.CanRead)
                {
                    //Daten lesen
                    var numberOfBytesRead = myNetworkStream.EndRead(ar);

                    //Wenn Daten vorhanden
                    if (numberOfBytesRead > 0)
                    {
                        //Event abschicken
                        if (DataReceived != null)
                        {
                            //Nur gelesene Bytes ermitteln
                            var data = new byte[numberOfBytesRead];
                            Array.Copy(m_ByteBuffer, 0, data, 0, numberOfBytesRead);

                            //Abschicken
                            DataReceived(this, data);
                        }
                    }
                    else
                    {
                        //Event abschicken
                        if (ClientDisconnected != null)
                            ClientDisconnected(this, "FIN");

                        //Wenn kein AutoConnect
                        if (m_AutoConnect == false)
                            disconnect_intern();
                        else
                            Disconnect_ButAutoConnect();

                        //Fertig
                        return;
                    }

                    //Neuer Lesevorgang
                    myNetworkStream.BeginRead(m_ByteBuffer, 0, m_ByteBuffer.Length, OnDataReceived, myNetworkStream);
                }
            }
            catch (Exception ex)
            {
                //Exception Event abschicken
                ExceptionAppeared(this, ex);
            }
        }

        /// <summary>
        ///     Neu Verbinden
        /// </summary>
        public void ReConnect()
        {
            //Verbindung beenden
            Disconnect();
            //Neue Verbindung starten
            Connect();
        }

        /// <summary>
        ///     Verbindung aufbauen
        /// </summary>
        public void Connect()
        {
            try
            {
                //Evtl. AutoConnect aktivieren
                InitTimerAutoConnect();

                // Erzeuge neuen Socket der an den m_Server und m_Port gebunden ist
                Client = new TcpClient(m_Server, m_Port);
                m_NetStream = Client.GetStream();

                //Beginn zu lesen
                StartReading();

                //Event abschicken
                ClientConnected(this, string.Format("server: {0} port: {1}", m_Server, m_Port));
            }
            catch (Exception ex)
            {
                //Weiterleiten
                throw ex;
            }
        }

        /// <summary>
        ///     Server des Clients anpingen
        /// </summary>
        public void Ping()
        {
            var ping = new Ping();
            var reply = ping.Send(m_Server);
            if (reply.Status != IPStatus.Success)
                throw new Exception(string.Format("Server {0} antwortet nicht auf Ping ", m_Server));
        }

        /// <summary>
        ///     Server des Clients anpingen. Angabe der maximalen Wartezeit in Millisekungen.
        /// </summary>
        /// <param name="waitTimeout"></param>
        public void Ping(int waitTimeout)
        {
            var ping = new Ping();
            var reply = ping.Send(m_Server, waitTimeout);
            if (reply.Status != IPStatus.Success)
                throw new Exception(string.Format("Server {0} antwortet nicht auf Ping ", m_Server));
        }

        /// <summary>
        ///     Verbindung beenden
        /// </summary>
        public void Disconnect()
        {
            //Verbindung beenden
            disconnect_intern();

            //Wenn nicht schon beendet
            if (m_TimerAutoConnect != null)
            {
                //Nicht mehr wiederverbinden
                m_TimerAutoConnect.Dispose();
                m_TimerAutoConnect = null;
            }

            //Event abschicken
            if (ClientDisconnected != null)
                ClientDisconnected(this, "Verbindung beendet");
        }

        /// <summary>
        ///     Verbindung beenden, aber AutoConnect beibehalten
        /// </summary>
        private void Disconnect_ButAutoConnect()
        {
            //Verbindung beenden
            disconnect_intern();
        }

        /// <summary>
        ///     Verbindung beenden (intern)
        /// </summary>
        private void disconnect_intern()
        {
            if (Client != null)
                Client.Close();
            if (m_NetStream != null)
                m_NetStream.Close();
        }

        /// <summary>
        ///     Timer der das automatische Verbinden steuert
        /// </summary>
        /// <param name="ob"></param>
        private void OnTimer_AutoConnect(object ob)
        {
            try
            {
                lock (Locker_AutoConnect)
                {
                    //Wenn gewünscht
                    if (m_AutoConnect)
                    {
                        //Wenn nicht verbunden
                        if (Client == null || Client.Connected == false)
                        {
                            //Erzeuge neuen Socket der an den m_Server und m_Port gebunden ist
                            Client = new TcpClient(m_Server, m_Port);
                            m_NetStream = Client.GetStream();

                            //Beginn zu lesen
                            StartReading();

                            //Event abschicken
                            ClientConnected(this, string.Format("server: {0} port: {1}", m_Server, m_Port));
                        }
                    }
                    else
                    {
                        if (m_TimerAutoConnect != null)
                        {
                            //Timer beenden
                            m_TimerAutoConnect.Dispose();
                            m_TimerAutoConnect = null;
                        }
                    }
                }
            }
            catch (Exception ex)
            {
                //Exception Event abschicken
                ExceptionAppeared(this, ex);
            }
        }

        /// <summary>
        ///     Wenn eine Exception passiert
        /// </summary>
        /// <param name="ex"></param>
        private void OnExceptionAppeared(TCPClient client, Exception ex)
        {
        }

        /// <summary>
        ///     Wenn sich ein Client verbunden hat
        /// </summary>
        /// <param name="client"></param>
        private void OnConnected(TCPClient client, string info)
        {
        }

        /// <summary>
        ///     Wenn sich ein Client getrennt hat
        /// </summary>
        /// <param name="client"></param>
        private void OnDisconnected(TCPClient client, string info)
        {
        }

        /// <summary>
        ///     Locker Klasse für AutoConnect
        /// </summary>
        private class Locker_AutoConnectClass
        {
        }
    }
}